...
ARBITRUMSCAN_NOVA_API_URL = "https://api-nova.arbiscan.io/api"

# Use API keys
API_KEYS_ARBITRUM_NOVA = [
    "API1",
    "API2",
    "API3",
    "API4",
...
def fetch_txlist_and_save(address, api_key):
    ...
    while True:
        time.sleep(1)
        response = requests.get(ARBITRUMSCAN_NOVA_API_URL, params=params)
        ...
        if response.status_code == 200:
            ...
            print(f"API error for address {address} with API key {api_key}. Retrying in 6s.")
            time.sleep(5)  # Wait for 6s
...
output_directory = "D:\\sybil\\arb_nova_txlists"
...
fetch_all_addresses_and_save_txlist(addresses_to_process, API_KEYS_ARBITRUM_NOVA, output_directory)
...
valid_addresses_df.to_csv(r"D:\sybil\arb_nova_valid_addresses.csv", index=False)
