...
valid_addresses_df = pd.read_csv("D:/sybil/fantom_valid_addresses.csv")
valid_addresses = set(valid_addresses_df['address'])

valid_pairs = set()

# 2. Open the file and process each .csv file inside
directory_path = "D:/sybil/fantom_txlists"
...
output_path = "D:/sybil/fantom_txlist_address_pairs.csv"
output_df = pd.DataFrame(list(valid_pairs), columns=["address_pair"])
output_df.to_csv(output_path, index=False)

print("Processing completed. Filtered address pairs saved to fantom_txlist_address_pairs.csv.")
