...
# Load the CSV file
data_all_groups = pd.read_csv("D:/sybil/arb_nova_group_transactions.csv")

output_directory = "arb_nova_photo"
...
def plot_graph_for_group(index):
    ...
    ax.set_title(f"Address Relationship for arb_nova_txlist_group{index} with Shell Layout", fontsize=16, pad=20)

    output_path = os.path.join(output_directory, f"arb_nova_txlist_group{index}.png")
