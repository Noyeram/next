...
# Load the CSV file
data_all_groups = pd.read_csv("D:/sybil/linea_group_transactions.csv")

output_directory = "linea_photo"
...
def plot_graph_for_group(index):
    ...
    ax.set_title(f"Address Relationship for linea_txlist_group{index} with Shell Layout", fontsize=16, pad=20)

    output_path = os.path.join(output_directory, f"linea_txlist_group{index}.png")
