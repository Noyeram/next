...
def fetch_txlist_and_save(address, api_key):
    ...
    while True:
        time.sleep(1)
        response = requests.get(ETHSCAN_API_URL, params=params)
        ...
        if response.status_code == 200:
            ...
            print(f"API error for address {address} with API key {api_key}. Retrying in 5s.")
...
output_directory = "D:\\sybil\\eth_txlists"
...
fetch_all_addresses_and_save_txlist(addresses_to_process, API_KEYS_ETH, output_directory)
...
valid_addresses_df.to_csv(r"D:\sybil\eth_valid_addresses.csv", index=False)
